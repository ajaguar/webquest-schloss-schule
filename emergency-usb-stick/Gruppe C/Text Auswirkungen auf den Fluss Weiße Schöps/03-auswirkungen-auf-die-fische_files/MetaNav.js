...
define("MetaNav", ['MobileMetaNavSubMenu'], function(MobileMetaNavSubMenu) {
    ...
    MetaNav.prototype.centerItem = function(item) {
        if (item)
        {
            ...
            if ((item_x + item_width > viewport_width && item_width < viewport_width) || (offset_x + item_x))
            {
    ...
    if ( Modernizr.touch )
    {
        MetaNav.prototype.initialize = function()
        {
            ...
            // close subnav
            this.scrollcontent.on('touchstart', 'li', function(e)
            {
                e.preventDefault();
    ...
    else
    {
        ...
        MetaNav.prototype.initTabKeyNavigation = function()
        {
            ...
            this.scrollcontent.on('focusin', 'li', function(event)
            {
                if (that.scroller)
                {
        ...
        {
            var that = this;
            var space_for_btn = 50;
            var scrollable = (this.content_width + space_for_btn) > this.dom_element.width();
            ...
            if (scrollable)
            {
                this.scrollcontent.css('width', this.content_width + space_for_btn);
